...
///
/// If `treat_name_as_symbol` is true, then we're not doing a pure text search but treat name
/// as a symbol. We apply a crude heuristic to ensure we have found the `name` symbol instead of
/// `name` as part of a larger string.
///
